...
func TestRegistryRoundTripAndPermissions(t *testing.T) {
	root := t.TempDir()
	dir := filepath.Join(root, "confluence-cli")
...
func TestRegistryFailsClosed(t *testing.T) {
	tests := []struct {
		...
		{name: "duplicate name", content: `[{"name":"work","site":"https://tenant.atlassian.net","email":"user@example.com","cloud_id":"cloud-id"},{"name":"work","site":"https://tenant.atlassian.net","email":"user@example.com","cloud_id":"cloud-id"}]`, mode: 0o600, wantErr: ErrCorruptRegistry},
		{name: "partial modern metadata", content: `[{"name":"work","site":"https://tenant.atlassian.net","email":"user@example.com","cloud_id":"cloud-id","capabilities":["read"]}]`, mode: 0o600, wantErr: ErrCorruptRegistry},
		{name: "insecure file permissions", content: `[]`, mode: 0o644, wantErr: ErrInsecurePermissions},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dir := filepath.Join(t.TempDir(), "confluence-cli")
...
func TestRegistryDoesNotOverwriteExistingProfile(t *testing.T) {
	registry := NewRegistry(filepath.Join(t.TempDir(), "confluence-cli", registryFilename))
	ctx := context.Background()
	if err := registry.Add(ctx, validProfile("work")); err != nil {
