...
const module = "github.com/abigotado/confluence-cli"
...
// Confluence credentials must be injected as values. Importing auth or profile here
// would couple every HTTP-client test to credential and filesystem machinery.
func TestConfluenceDoesNotImportAuthOrProfile(t *testing.T) {
	if !packageExists(t, "internal/confluence") {
		t.Skip("internal/confluence has not been added yet")
	}
	forbidden := map[string]bool{
		module + "/internal/auth":    true,
		module + "/internal/profile": true,
	}
	for _, dependency := range dependencies(t, module+"/internal/confluence") {
		if forbidden[dependency] {
			t.Errorf("internal/confluence depends on %s; inject non-secret config and credentials as values", dependency)
		}
	}
}

// CLI commands delegate transport behavior to internal/confluence so pacing,
...
func TestCLIDoesNotImportNetHTTP(t *testing.T) {
	...
	for _, imported := range directImports(t, module+"/internal/cli") {
		if imported == "net/http" {
			t.Error("internal/cli imports net/http; add the operation to internal/confluence")
		}
	}
}
