...
//go:generate go run github.com/abigotado/confluence-cli/tools/gencontract
...
// Code is a confluence-cli process exit status.
//
...
const (
	...
	CodeOK Code = 0
	// CodeInternal signals a confluence-cli defect.
	CodeInternal Code = 1
	// CodeUsage signals invalid flags or input.
	CodeUsage Code = 2
	// CodeNotFound signals an absent or invisible Confluence entity.
	CodeNotFound Code = 3
	...
	CodeConfirm Code = 7
	// CodePermission signals a Confluence permission or API-token scope denial.
...
var codes = []CodeInfo{
	...
	{CodePermission, "PERMISSION_DENIED", "Confluence permission or token scope denied", "request permission or scope; do not retry unchanged"},
	{CodeConflict, "CONFLICT", "stale state or write conflict", "re-read the Confluence resource before deciding"},
}
...
// Contract is the machine-readable description emitted by `confluence-cli contract`.
