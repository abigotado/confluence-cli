...
func TestExitCode(t *testing.T) {
	tests := []struct {
		...
		{"not found", NotFound("page", "123", nil), CodeNotFound},
		{"ambiguous", Ambiguous("project", "W", nil), CodeAmbiguous},
		{"write applied local failure", WriteAppliedLocalFailure("pages.update"), CodeInternal},
		...
		{"confirm", ConfirmRequired("update page"), CodeConfirm},
		...
		{"conflict", Conflict("STALE_PAGE", "page changed"), CodeConflict},
...
func TestConfluenceSpecificHints(t *testing.T) {
	tests := []struct {
		name     string
		err      *Error
		wantCode Code
		...
		{"page not found", NotFound("page", "404", nil), CodeNotFound, "--profile"},
