// Package output renders confluence-cli results while preserving the v1 machine
...
type Meta struct {
	...
	// NextCursor is Confluence's opaque next-page token.
	NextCursor string `json:"next_cursor,omitempty"`
	// Profile is the selected non-secret profile name.
	Profile string `json:"profile,omitempty"`
	// Site is the selected Confluence tenant URL.
...
// WithContext adds the selected non-secret profile and Confluence site to success
// metadata. It mutates the writer and returns it for command setup chaining.
func (w *Writer) WithContext(profile, site string) *Writer {
	w.profile = profile
	w.site = site
	return w
}
...
// when Confluence reports no subsequent page.
...
func (w *Writer) Failure(err error) errx.Code {
	...
	hint := "this is a bug in confluence-cli; do not retry unchanged"
