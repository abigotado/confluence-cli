...
def _runs_quality_gate(
    ...
    """Report whether this pre-push hook actually runs the confluence-cli gate.
...
def _not_installed_reason(configured: Optional[str]) -> str:
    if configured and configured != _TRACKED_HOOKS_PATH:
        return (
            f"confluence-cli pre-push gate does not run: core.hooksPath is "
    ...
    return (
        "confluence-cli pre-push hook is not installed. "
...
def main() -> int:
    try:
        ...
        _deny("confluence-cli push guard received invalid hook input; command denied.")
        return 0
    ...
    if invocation.overrides_hooks_path:
        _deny(
            "confluence-cli does not allow overriding core.hooksPath for git push. "
    ...
    if any(_is_no_verify_option(argument) for argument in invocation.arguments):
        _deny(
            "confluence-cli does not allow git push --no-verify. "
    ...
    if repository_root is None:
        _deny("confluence-cli push guard cannot resolve the repository; command denied.")
        return 0
