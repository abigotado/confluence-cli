...
if ! "$python_bin" -c 'pass' >/dev/null 2>&1; then
  echo "[confluence-cli-go-format] WARN: interpreter '$python_bin' is not runnable; hook skipped" >&2
  exit 0
fi
...
if [[ -z "$repo_root" ]]; then
  echo "[confluence-cli-go-format] WARN: cannot resolve repository root" >&2
  exit 0
fi
...
if ! command -v gofmt >/dev/null 2>&1; then
  echo "[confluence-cli-go-format] WARN: gofmt not found; skipped formatting" >&2
  exit 0
fi
...
if ! gofmt -w "$target" >/dev/null; then
  echo "[confluence-cli-go-format] WARN: gofmt failed for $target" >&2
fi
exit 0
