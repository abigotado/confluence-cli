---
name: write-tests
description: Add or update confluence-cli Go tests for changed behavior. Use after implementing a change, or when coverage is missing for a command, the Confluence client, profile or credential handling, or the exit-code and envelope contract.
---

# Write confluence-cli Tests
...
4. Mock at boundaries only — `httptest` for HTTP, an injected fake
   ...
   Never reach the real Confluence API or an existing Keychain item.
