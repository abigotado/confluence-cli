...
`assets/skills/confluence/reference/contract.md` are generated from `internal/errx`
...
4. The sole emergency markers are the corresponding exact, anchored, complete
   ...
   applied, but local finalization failed`. Neither is accepted as a substring,
   partial line, mid-line value, or for another command. An exact line means the
   write is known applied and must not be retried.
5. All other stderr is diagnostic. A confirmed write with invalid stdout and no
   exact marker gets bounded reconciliation reads, but the caller never retries
   automatically or claims the write applied. Other commands report invalid
   machine output without inferring state from stderr.
...
| 9 | conflict / stale state | re-read the Confluence object before retrying |
...
- Guarded page writes and local write-policy changes accept `--dry-run` and
  `--yes`, registered by the shared command helper rather than per command.
  Credential login/logout require `--yes` when confirmation is needed but do
  not support previews: login cannot verify a token without performing remote
  checks, and a logout preview would not prove Keychain removability.
...
**not** a passthrough of Confluence's response: the client decodes into its own
