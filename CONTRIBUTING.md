...
`confluence-cli` is a single Go binary whose command surface is consumed far more
...
`-race` is not optional here. The Confluence client paces and retries from shared
...
**`docs/commands.md`, `docs/contract.md`, and `assets/skills/confluence/reference/`**
...
- **Dependency direction is verified, not assumed.** `internal/cli` never calls
  `net/http`, `internal/confluence` never imports `internal/auth`, and
  `internal/errx` imports nothing else from this module.
- **Every network command requires `--profile`.** There is no active profile
  switch. Future page mutations must follow `docs/guarded-writes.md`, including
  capability migration, local-only dry-run, confirmation, and a bound space
  allowlist.
...
and `t.Setenv()` for the environment. Tests never reach Confluence Cloud, the
real OS Keychain, or the developer's home directory.

Do not weaken an assertion to make a failing test pass. Report the failure.

## Credentials

This binary holds a Confluence API token that grants the permissions of its Atlassian
